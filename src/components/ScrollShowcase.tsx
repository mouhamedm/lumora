...
export default function ScrollShowcase() {
  ...
  return (
    <section
      id="manifesto"
      ref={containerRef}
      className="relative py-20 sm:py-32 bg-[#FAFAF8] overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        {/* Section Header */}
        <div className="mb-14 pb-6 border-b border-[#111110]/10 flex flex-wrap items-center justify-between gap-4">
          <span className="text-xs font-mono font-bold tracking-widest uppercase text-[#FF3B1D]">
            [ 02 // MANIFESTE & VISION DIGITALE ]
          </span>
          <span className="text-xs font-mono text-[#6E6E6A] tracking-wider uppercase">
        ...
        <div className="relative mb-24 flex justify-center">
          <motion.div
            ...
            className="relative w-full aspect-[16/10] sm:aspect-[21/9] overflow-hidden shadow-2xl shadow-black/10 border border-[#111110]/10 bg-[#111110]"
            onMouseEnter={() => setCursor("project", "DISCOVER")}
            onMouseLeave={resetCursor}
          >
            {/* Background Cinematic Visual with Scroll-Driven Parallax Zoom */}
            <motion.div
              style={{
                scale: imageScale,
                backgroundImage:
            ...
            <div className="absolute inset-0 p-6 sm:p-10 flex flex-col justify-between text-white z-10 pointer-events-none">
              ...
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                  <span className="text-xs font-mono uppercase text-[#FF3B1D] font-bold block mb-1">
        ...
        {/* Scroll-Driven Typography: Words Illuminating Progressively */}
        <div className="max-w-4xl mx-auto my-12 sm:my-20">
          <div className="flex flex-col gap-6 sm:gap-8">
            {MANIFESTO_LINES.map((line, idx) => {
              // Compute unique scroll range for each line to illuminate sequentially
  ...
  return (
    <motion.p
      ...
      className={`text-2xl sm:text-4xl md:text-5xl font-black uppercase tracking-tight transition-colors duration-200 ${
        line.highlight ? "text-[#FF3B1D]" : "text-[#111110]"
      }`}
    >
      {line.text}
    </motion.p>
